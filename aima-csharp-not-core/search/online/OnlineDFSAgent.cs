...
namespace aima.core.search.online
{
	...
	public override Action execute(Percept psDelta)
	{
		...
		// ACTIONS(s')
		if (!untried.TryGetValue(sDelta, out List<Action>? value))
		{
            value = actions(sDelta);
            untried.Add(sDelta, value);
		}
		...
		if (null != s)
		{
		    ...
		    if (!(sDelta.Equals(result.get(s, a))))
		    {
				...
				if (!unbacktracked.TryGetValue(sDelta, out List<object>? value1))
				{
					value1 = new List<object>();
					unbacktracked.Add(sDelta, value1);
				}
